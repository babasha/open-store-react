...
const OrderItem: React.FC<Props> = ({ order, setOrders, disableTimers }) => {
  ...
  const handleStatusChange = async (id: number, status: string) => {
    try {
      ...
      await axios.put(`http://45.146.164.162:3000/orders/${id}/status`, { status, status_changed_at: updatedStatusChangedAt }, { withCredentials: true });
  ...
  const handleCancelOrder = async (id: number) => {
    try {
      const updatedStatusChangedAt = new Date().toISOString();
      await axios.put(`http://45.146.164.162:3000/orders/${id}/status`, { status: 'canceled', status_changed_at: updatedStatusChangedAt }, { withCredentials: true });
      setOrders((prevOrders) =>
        prevOrders.map((order) =>
  ...
  const handleConfirmChange = async (productId: number, newQuantity: number) => {
    try {
      ...
      await axios.put(`http://45.146.164.162:3000/orders/${order.id}/items`, { items: updatedItems }, { withCredentials: true });
      setOrders((prevOrders) =>
        prevOrders.map((ord) =>
  ...
  const handleDeliveryModeChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      await axios.put(`http://45.146.164.162:3000/orders/${order.id}/delivery-option`, { deliveryOption: newDeliveryOption }, { withCredentials: true });
